...
% 
function [mass_rangeU, CfixedU,nppG,npp_og]=CfixedRespTotalTime(sim,rates,time,intPoints,lat,lon)
...
Cfixed_G(ixG) = rates.jLreal(time,ixG).*sim.B(time,ixG);       % fixed carbon  

% diatoms
[~, idxD]  = find(sim.p.typeGroups==3); 
ixD        = (sim.p.ixStart( idxD (1)):sim.p.ixEnd( idxD (end)))-sim.p.idxB+1;% index of diatoms
% mD         = p.m(ixD);   
Cfixed_G(ixD) = rates.jLreal(time,ixD).*sim.B(time,ixD);        % fixed carbon
% mUni       = [mG,mD];
...
%    case 'Generalists'
    iGroup = 1;
        ...
        jResp(ixG) = rates.jR(time,ixG) + betaL*rates.jLreal(time,ixG) + ( betaN*rates.jN(time,ixG).*rates.jLreal(time,ixG)./(rates.jLreal(time,ixG)+rates.jDOC(time,ixG)))...
            + (betaG*jnet(time,ixG).*rates.jLreal(time,ixG)./(rates.jLreal(time,ixG)+rates.jDOC(time,ixG)+rates.jFreal(time,ixG)));
    % case 'Diatoms'
    iGroup = 2;
        % Find beta parameters from the input file:
        betaL = search_namelist('../input/input.h', p.nameGroup{iGroup}, 'bL');
        betaN = search_namelist('../input/input.h', p.nameGroup{iGroup}, 'bN');
        betaG = search_namelist('../input/input.h', p.nameGroup{iGroup}, 'bg');
        jResp(ixD) = rates.jR(time,ixD) + betaL*rates.jLreal(time,ixD) + ( betaN*rates.jN(time,ixD).*rates.jLreal(time,ixD)./(rates.jLreal(time,ixD)+rates.jDOC(time,ixD)))...
            + (betaG*jnet(time,ixD).*rates.jLreal(time,ixD)./(rates.jLreal(time,ixD)+rates.jDOC(time,ixD)));
    
        jResp(isnan(jResp))=0;
    nppG = abs(Cfixed_G - jResp);

    % filename='../input/input.h';
    fclose('all');
%% Step1: Interpolation 
...
for iGroup = 1:2            % check for only 2 groups
    ...
    val_to_interp2 = jResp(ixB).*sim.B(time,ixB) ;
...
npp_og=sum(Cfixed_G-jResp.*sim.B(time,ixG(1):ixD(end)));
...
% % hold on
% %     plot(m,sim.rates.mortHTL.*sim.B(time,:)','o')
% % 
% % xlabel('Mass (\mugC)')
