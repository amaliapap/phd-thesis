...
%                    2) Global functions
%
function plotsForTrophicLevels(sim,mte)
...
% Annual averges at top layer
    mNPP_avg_yr=squeeze(mean(mNPP,1));
    mHTL_avg_yr=squeeze(mean(mHTL,1));
    % lambdaHTL_avg_yr=squeeze(mean(lambda_htl,1,'omitnan'));
    lambda_avg_yr=squeeze(mean(TLtimeTop170,1));
% for iTime=1:12
%     mNPP_acc_winter=NPP(iTime,:,:).*mNPP(iTime)/NPP(iTime);
% end
...
pte=mte.^(1./(lambdaHTL-1));
pte(isnan(pte))=0;
%
lambdaHTL4d=load('C:\Users\ampap\OneDrive - Danmarks Tekniske Universitet\Documents\GitHub\NUMmodel\matlab\Trophic Efficiency\27 Aug 2024 plots\lambdaHTL4d.mat');
lambda_HTL4d=squeeze(mean(lambdaHTL4d.lambdaHTL(:,:,:,1),1));% annual avg at top 5m
%% ---------------------------------------
...
'PaperPositionMode','auto','Name','TrophicLvl Diagnostics: Top 5m');
clf
set(gcf,'color','w');
tiledlayout(2,2,'TileSpacing','compact','padding','compact')

t1=nexttile(1);                                 % only top layer
c = panelGlobal(sim.x,sim.y,log(mean(mNPP_avg_yr(:,:,1),3)),[-8 1],sTitle='a. Annual average mass_{NPP}',sProjection='mollweid');
    c.Visible='off';
    colormap(t1,ccmap)
...
%%
    t2=nexttile(2);
    c = panelGlobal(sim.x,sim.y,log(mean(mHTL_avg_yr(:,:,1),3)),[-8 2],sTitle='b. Annual average mass_{HTL}',sProjection='mollweid');
    c.Visible='off';
    colormap(t2,ccmap)
    c=colorbar('horizontal');
    set(gca,'YTickLabel',[]);
...
figure(100)
clf
c = panelGlobal(sim.x,sim.y,log(mNPP_norm_winter),[-8 1],sTitle='Normalized annual mass_{NPP}',sProjection='mollweid');
    c.Visible='off';
    colormap(t2,ccmap)
    c=colorbar('horizontal');
    set(gca,'YTickLabel',[]);
...
t1=nexttile(1);
c = panelGlobal(sim.x,sim.y,log(mean(mNPP_avg_yr,3,'omitnan')),[-8 1],sTitle='Annual average mass_{NPP}',sProjection='mollweid');
    c.Visible='off';
    colormap(t1,ccmap)
    c=colorbar('horizontal'); 
    set(gca,'YTickLabel',[]);
